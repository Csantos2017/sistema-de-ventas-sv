...
function editarProductoHandler(id) {
  ...
  showModal(`
    ...
    <form id="formEditProd">
      ...
      <div class="form-group"><label>Categoría</label>
        <input name="categoria" value="${prod.categoria}"></div>
      <div class="form-group"><label>Costo de Compra</label>
      ...
      <div id="promoCampos" style="${prod.promoActiva ? 'display:block;' : 'display:none;'}">
        <div class="form-group"><label>Tipo</label>
          <select name="promocionTipo">
        ...
        <div class="form-group"><label>Cantidad</label>
          <input name="promocionCantidad" type="number" min="0" value="${prod.promocionCantidad || 0}">
        </div>
      </div>
      <div style="text-align:right;">
        <button type="button" onclick="window.closeModal()">Cancelar</button>
  ...
  `);
  document.getElementById('formEditProd').onsubmit = function(ev) {
    ev.preventDefault();
    const datos = Object.fromEntries(new FormData(this).entries());
    datos.id = prod.id;
    datos.costoCompra = +datos.costoCompra;
